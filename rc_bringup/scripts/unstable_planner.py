...
def unstable_planner2D(Obs_xy, r, goal, x, y, phi, planning_distance):
    ...
    if beta_max!=-100:
        ...
        if drg>rc_min:
            ...
            Rx=1*math.cos(phi)
            Ry=1*math.sin(phi)
            angle_robot_Obs=math.acos((Ox*Rx+Oy*Ry)/(math.sqrt(Ox**2+Oy**2)*(Rx**2+Ry**2)))
            acos((Ox * Rx + Oy * Ry) / (sqrt(Ox ^ 2 + Oy ^ 2) * (Rx ^ 2 + Ry ^ (2))))
...
if __name__ == "__main__":
    ...
    try:
        while not rospy.is_shutdown():
            ...
            goal_pose_msg= unstable_planner2D(cloud_of_np_global,1, goal_new_p, current_pose.pose.position.x, current_pose.pose.position.y, current_course, 1.5)
            step_1=step_1+1
            if finish_flag:
                if currentTime > 1.0:
